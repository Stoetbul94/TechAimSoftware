# -*- coding: utf-8 -*-
"""DEPLOY-001 — a deployed SETA runtime folder is self-contained and clean.
...
Run: python tests/release/check_deployment.py <deployment-dir>
"""
import hashlib
import json
import os
...
def main():
    if len(sys.argv) < 2:
        print('usage: check_deployment.py <deployment-dir>')
        return 2
    ...
    for mod in ('QtQuick', 'QtQml', 'QtQuick/Controls', 'QtQuick/Layouts',
                'QtQuick/Window', 'QtCharts', 'QtMultimedia'):
        check(os.path.isdir(os.path.join(root, 'qml', *mod.split('/'))),
              'QML module deployed: %s' % mod)
    for plug in ('imageformats', 'tls', 'multimedia', 'styles'):
    ...
    check(product is not None and product.startswith('SETA'),
          'the deployed executable identifies as SETA in its VERSION RESOURCE '
          '- which is what Explorer and Properties show', str(product))
    check(product is None or 'Tech Aim' not in product,
          'and its product name is not Tech Aim', str(product))
    check('JAC SHOOTING SOLUTIONS (PTY) LTD'.encode('utf-16-le') in raw,
          'the legal publisher is unchanged')
    seta_ico = os.path.join(os.path.dirname(os.path.dirname(
        os.path.dirname(os.path.abspath(__file__)))), 'images', 'logo', 'seta.ico')
    if os.path.isfile(seta_ico):
        with open(seta_ico, 'rb') as f:
            ico = f.read()
        count = struct.unpack('<H', ico[4:6])[0]
        biggest = max((struct.unpack('<II', ico[6 + i * 16 + 8:6 + i * 16 + 16])
                       for i in range(count)), key=lambda e: e[0])
        blob = ico[biggest[1]:biggest[1] + biggest[0]]
        check(blob in raw,
              'the SETA icon is embedded in the deployed executable - the shell '
              'shows the product mark, not the Tech Aim one')
    ...
    state = [f for f in os.listdir(root)
             if f.lower() in ('config.ini', 'userhistory.txt', 'userdetails.txt',
                              'userdetails_seta.txt')
             or f.lower().startswith('match_')]
    check(not state,
          'no athlete, session, port or configuration state from the build '
          'machine is deployed - the first run starts clean',
          ', '.join(sorted(state)[:6]))
