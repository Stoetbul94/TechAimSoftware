...
int main(int argc, char* argv[])
{
    ...
    // ── control channel: REAL, not status-only ───────────────────
    // The first physical test found RMS detecting a lost shot correctly and
    // being unable to do anything about it, because the qualified control
    // client had never been attached to the running application. It is now.
    //
    // THE KEY IS LOADED, NEVER CREATED. It is generated by the node and copied
    // to this laptop. If RMS minted its own when the file was missing, both
    // ends would hold a valid-looking key that simply did not match, and every
    // lane would report AUTH FAILURE - a configuration mistake wearing the
    // costume of a security failure.
    QString rangeKeyError;
    const QString rangeKeyPath = rmsDataFile(QStringLiteral("range.key"));
    const QByteArray rangeKey =
        ta::rms::control::loadRangeKey(rangeKeyPath, &rangeKeyError);
    const bool keyConfigured = rangeKey.size() >= 32;

    ta::rms::control::NodeAddressBook addressBook;
    ta::rms::control::RangeControlCoordinator coordinator(
        QStringLiteral("RMS-%1").arg(QSysInfo::machineHostName()), rangeKey);
    ta::rms::control::RmsControlTransport controlTransport(&addressBook);
    coordinator.setLink(controlTransport.link());
    // The node builds one endpoint per connection, so each handshake must
    // arrive on a fresh socket.
    coordinator.setPreConnectHook([&controlTransport](const QString& nodeId) {
        controlTransport.dropPeer(nodeId);
    });
    QObject::connect(&controlTransport,
                     &ta::rms::control::RmsControlTransport::transportProblem,
                     [](const QString& nodeId, const QString& reason) {
                         std::fprintf(stderr, "RMS: control %s - %s\n",
                                      qPrintable(nodeId), qPrintable(reason));
                     });

    ta::rms::control::ControlStatusModel controlStatus;
    // The coordinator is attached ONLY when a key exists. Without one there is
    // nothing it could successfully do, and a panel showing live-looking
    // control state would be the lie this whole design avoids.
    controlStatus.setSources(&monitor, keyConfigured ? &coordinator : nullptr);
    controlStatus.setTransportAttached(true);
    controlStatus.setKeyConfigured(keyConfigured, rangeKeyError);
    if (keyConfigured)
        std::fprintf(stderr, "RMS: control key loaded from %s\n",
                     qPrintable(rangeKeyPath));
    else
        std::fprintf(stderr,
                     "RMS: control NOT configured - %s (expected at %s)\n",
                     qPrintable(rangeKeyError), qPrintable(rangeKeyPath));
    ...
    if (live) {
        QObject::connect(&observer, &RmsUdpObserver::datagramReceivedFrom,
                         [&](const QByteArray& d, const QHostAddress& from) {
                             const qint64 now = QDateTime::currentMSecsSinceEpoch();
                             // ONE ingress, unchanged. The address is learned
                             // ALONGSIDE it, never through it: the address book
                             // reads only the nodeId and has no opinion about
                             // what a datagram means.
                             monitor.ingestDatagram(d, now);
                             addressBook.observe(d, from, now);
                         });
    ...
    // ── automatic catch-up ───────────────────────────────────────────────
    // NO OPERATOR BUTTON. The first physical test lost one middle datagram and
    // RMS detected it correctly; an operator should never have to press
    // "recover shots" for that. serviceNodes() reauthenticates whatever needs
    // it, retries pending commands under their original ids, and reconciles
    // every lane that is behind.
    //
    // ON ITS OWN, SLOWER TIMER. The control transport is synchronous with a
    // bounded timeout, so it must not share the 250 ms dashboard tick - a
    // stalled node would stutter the UI. Two seconds is far below any human
    // sense of delay and far above the cost of a healthy round trip.
    QTimer controlTick;
    if (live && keyConfigured) {
        QObject::connect(&controlTick, &QTimer::timeout, [&]() {
            coordinator.serviceNodes(&monitor, QDateTime::currentMSecsSinceEpoch());
            controlStatus.refresh();
        });
        controlTick.start(2000);
        std::fprintf(stderr, "RMS: automatic catch-up active (control tick 2 s)\n");
    } else if (live) {
        std::fprintf(stderr,
                     "RMS: automatic catch-up NOT active - control is not configured\n");
    }
