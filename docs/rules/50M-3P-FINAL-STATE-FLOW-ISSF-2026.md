...
| Print | **First Print 12/2025** — as supplied by the project owner |
| Effective | 1 January 2026 |
| Rule | **6.17.3 — FINALS 50 m RIFLE 3-POSITION MEN AND WOMEN** |
| Course | **35 official shots** |

**Status of every rule statement below: 📋 user-supplied ISSF rule extract.**
Per [docs/issf-rules/README.md](../issf-rules/README.md) that tag means
"provided by the project owner as an ISSF rule; not yet independently verified
against a published PDF; treated as authoritative for implementation." No
copy of the rulebook is available to this repository, so nothing here carries
the ✅ tag.

### ⚠ Two source discrepancies, recorded rather than resolved

1. **Print mismatch.** This file cites *First Print 12/2025, effective
   1 January 2026*. `src/finals/Finals3PConfig.h` has always cited *Second
   Print 07/2026, effective 1 July 2026*, and the 10 m Finals rule files
   ([10m-finals-shared.md](../issf-rules/10m-finals-shared.md)) are ✅-verified
   against that later print. The durations agree, so nothing is blocked — but
   **which print governs has not been established** and should be settled
   before the next rules review. ⏳
2. **Prior authority was never edition-stamped.**
   [docs/issf-rules/50m-rifle-3p-final.md](../issf-rules/50m-rifle-3p-final.md)
   records *"Applicable ISSF rulebook edition: ⏳ not supplied"* and tags most
   of its content 🔧 *existing TechAim behaviour* — which the README explicitly
   says is "descriptive, **not** an authority — may be wrong." This file is the
   first edition-stamped normative statement of the 3P Final flow. ⏳ → 📋
...
## Tie-breaking — ⏳ AWAITING OFFICIAL RULE

The tie-breaking provisions of Rule 6.17.3 **were not supplied and are not
available to this repository.** Nothing about ties is implemented, inferred or
documented here, because inventing tie logic would be inventing a competition
rule.

Specifically unresolved:

- a tie at an elimination position during the singles
- a tie affecting the two finalists eliminated after Standing Series 2
- countback cases the rule provides explicitly
- whether tie-breaking shots are required, and their timing

**Required before any tie handling is written: the text of 6.17.3's
tie-breaking provisions.** Where tie resolution depends on other lanes, the
single-target application represents its own lane state and does not decide.
