...
| 2026-07-21 | **Phase E integration:** qualification reacts to the incident workflow (clock freeze on raise, sighter routing for recovery sighting, resume rebase to frozen+credit); officials refused at the controller while blocked; `recoveredRemainingMs` is pause/credit-aware; incident-aware resume opens the Jury window. | ShootingPage, CenterPane, QUAL |
