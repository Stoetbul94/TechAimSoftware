...
// Loads an EXISTING range key and never creates one.
//
// This is what RMS uses, and the difference matters. The key is generated by
// the NODE and copied to the range laptop; if RMS minted its own when the file
// was missing, both ends would hold a valid-looking 32-byte key that simply did
// not match, and every lane would report AUTH FAILURE - a configuration mistake
// wearing the costume of a security failure. Absent means NOT CONFIGURED, and
// that is what the operator is told.
QByteArray loadRangeKey(const QString& path, QString* errorOut = nullptr);
