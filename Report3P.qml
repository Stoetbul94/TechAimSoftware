...
Item {
    ...
    // ── competition identity, from the SESSION and not the catalogue ────
    // A result belongs to the rules it was shot under. window.sessionCompetition
    // is the adopted authority persisted with the match; when it is absent the
    // report keeps its existing ISSF wording, which is what every pre-DSB
    // result is.
    readonly property var auth: (typeof window !== "undefined"
                                 && window.sessionCompetition) ? window.sessionCompetition : null
    // "DSB 2026 · Rule 1.60 · 3x40 · DM 2026", assembled from machine values.
    // The ruleset edition is shown as its year: the stamp is 2026-01-01 and a
    // date on a report line reads as the match date.
    readonly property string competitionLine: {
        if (auth === null)
            return qsTr("50m Rifle 3 Positions · ISSF 3x20 Qualification")
        var parts = [qsTr("50m Rifle 3 Positions")]
        var fed = auth.rulesetId ? auth.rulesetId.toUpperCase() : ""
        var edition = auth.rulesetVersion ? auth.rulesetVersion.substring(0, 4) : ""
        if (fed !== "") parts.push(fed + (edition !== "" ? " " + edition : ""))
        if (auth.ruleNumber) parts.push(qsTr("Rule %1").arg(auth.ruleNumber))
        if (auth.programmeVariant) parts.push(auth.programmeVariant)
        if (auth.competitionContext) parts.push(auth.competitionContext.replace("_", " "))
        parts.push(auth.scoringMode === "INTEGER" ? qsTr("Whole ring")
                                                  : qsTr("Decimal"))
        return parts.join("  ·  ")
    }
    ...
    Rectangle {
        ...
        Column {
            ...
            Item {
                ...
                Column {
                    ...
                    Text {
                        text: reportRoot.competitionLine + (appMode ? "" : qsTr("   —  DEMO"))
        ...
        Item {
            ...
            Text {
                ...
                // The footer names the RULE AUTHORITY the result was shot
                // under, so a printed sheet cannot be read as an ISSF result
                // when it was a DSB one.
                text: reportRoot.auth === null
                      ? qsTr("Generated by %1 · ISSF 2026").arg(PRODUCT.fullProductName)
                      : qsTr("Generated by %1 · %2 %3 · %4")
                          .arg(PRODUCT.fullProductName)
                          .arg(reportRoot.auth.rulesetId ? reportRoot.auth.rulesetId.toUpperCase() : "")
                          .arg(reportRoot.auth.rulesetVersion
                               ? reportRoot.auth.rulesetVersion.substring(0, 4) : "")
                          .arg(reportRoot.auth.targetStandardId)
